...
const ShopOrderCollectionModal: React.FC<IModal<string>> = (props) => {
  const { openNotification } = useNotification();
  const { showModal, onClose, data: id } = props;
  const { data } = orderApi.useGetSingleOrderCollectionQuery(id!);

  const [confirmAgent, { isSuccess, isLoading, isError, error, data: res }] = orderApi.useConfirmAgentKeyMutation();
  ...
  return (
    ...
    >
      ...
      <div className="w-full py-3 max-h-[460px] overflow-x-auto">
        ...
        <div className={`px-8 py-5 border-t-2 border-cash-get-dark-100 space-y-5 ${data?.data?.agentConfirmed && "opacity-50"} `}>
...
export default ShopOrderCollectionModal;
